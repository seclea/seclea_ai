...
class Director:
    ...
    def cache_upload_object(
        self, obj_tracked: Tracked, obj_bs: BaseModel, api: BaseModelApi, params: dict
    ):
    ...
    def _store_entity(
        self, obj_tr: Tracked, obj_bs: BaseModel, api: BaseModelApi
    ) -> None:  # TODO add return for status
        """
        @param obj_tr:
        @param obj_bs:
        @return:
        ...
        future = self.write_threadpool_executor.submit(
            self.writer.cache_object, obj_tr=obj_tr, obj_bs=obj_bs, api=api
        )
        self.write_executing.add(future)
        future.add_done_callback(self._callback_write)

    def _send_entity(
        self, api: BaseModelApi, obj_bs: BaseModel, params: Dict
    ) -> None:  # TODO add return for status
        """
    ...
    def _ensure_error_count_0(self):
        ...
        raise DirectorException(
            f"Errors processing {len(self.errors)} requests to director, please check error logs."
        )
