...
class TestRegressionDataIntegration(TestCase):
    ...
    def step_0_project_setup(self):
        ...
        self.project_name = "Regression Project"
    ...
    def step_1_upload_dataset(self):
        self.sample_df = pd.read_csv(
            f"{folder_path}/data/energydata_complete.csv", index_col="date"
        )
    ...
    def step_3_upload_sklearn_training_run(self):
        ...
        sklearn_model = RandomForestRegressor(random_state=42, n_estimators=32)
        sklearn_model.fit(self.X_train, self.y_train)
        self.controller.upload_training_run_split(
            sklearn_model,
            X_train=self.X_train,
    ...
    def step_4_upload_xgboost_training_run(self):
        xgb_dtrain = DMatrix(data=self.X_train, label=self.y_train)
        params = dict(max_depth=3, eta=1, objective="reg:squarederror")
        num_rounds = 5
    ...
    def step_5_upload_lgbm_training_run(self):
        ...
        params = dict(max_depth=3, eta=1, objective="regression")
        num_rounds = 5
        # train model
        lgb_model = lgb.train(
            params=params,
